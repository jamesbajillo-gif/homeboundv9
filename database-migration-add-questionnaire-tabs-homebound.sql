-- Migration: Add questionnaire support to homebound custom tabs
...
-- Add tab_type, questionnaire_script_name, and selected_section_ids columns to homebound_custom_tabs
-- Check if column exists before adding (MySQL doesn't support IF NOT EXISTS for ALTER TABLE, so we use a workaround)
SET @dbname = DATABASE();
SET @tablename = 'homebound_custom_tabs';
...
-- Add same columns to homebound_listid_custom_tabs for List ID specific tabs
SET @tablename = 'homebound_listid_custom_tabs';
